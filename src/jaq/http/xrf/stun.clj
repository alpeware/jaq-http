   ...
   (def x
     (let [xf (comp
       ...
       (->> [{:context/bip-size (* 1 4096)
              ...
              :http/local-port 8080 #_1000 #_0 #_(-> fpp.server/env :PORT (Integer/parseInt))
              }]
            (into [] xf)
            (first))))

   (keys fpp.server/env)
   (-> fpp.server/env :PORT (Integer/parseInt))
